...
ESX.RegisterCommand('setfaction', 'admin', function(xPlayer, args, showError)
	if ESX.DoesFactionExist(args.faction, args.grade) then
		...
		showError(_U('command_setfaction_invalid'))
	end
end, true, {help = _U('command_setfaction'), validate = true, arguments = {
	{name = 'playerId', help = _U('commandgeneric_playerid'), type = 'player'},
	{name = 'faction', help = _U('command_setfaction_faction'), type = 'string'},
	{name = 'grade', help = _U('command_setfaction_grade'), type = 'number'}
}})
